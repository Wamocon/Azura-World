...
2. **Verified technical state** — a table with a column that says _what the evidence actually is_
3. **Functional limits of synthetic QA** — per feature, the honest current proof. Where the
   reference says _"Seed- oder Prozessdaten sind kein Persistenznachweis"_, say the equivalent.
...
- **A gate passing for the wrong reason** — e.g. e2e green because every test was skipped.
  Assert on the _executed_ count, not just the exit code.
...
- The single-sentence honest answer to: _can this be shown to a client on 29 July?_

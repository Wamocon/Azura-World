...
_Every Claude Code window working on this repository loads this file first. It overrides
default behaviour. Where it conflicts with a task brief, this file wins — except that a task
brief may make a rule **stricter**, never looser._
...
2. **Never silently resolve a conflict between sources.** When sources disagree, both values,
   ...
   _display_ value; the losing value is still stored and still visible on demand.
...
4. Generated files carry a header naming the generator script and saying _do not hand-edit_.
...
9. The AI system prompt must forbid the model from _executing_ financial, access-control, or
...
documents it as a repeated past failure (`LESSONS-LEARNED.md`: _"Test-Exit-Codes hinter `tail`
verstecken"_ — hiding exit codes behind a pipe).
...
- [ ] Task-specific verification commands in the brief pass _(paste the output)_

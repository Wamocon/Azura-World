...
Five layout shifts between 677 ms and 1080 ms. Four of them alternate between the _same two_
...
The file's own comment says: _"an entry pointing at a missing file is worse than a sparse icon
list, because it fails installability silently."_ That is precisely the state it is in. It is the
...
The clipping case is the brief's own thesis landing: _"German and Russian are where this finds
bugs."_ It found Turkish and Russian, on the honesty control — the `MODELLED` badge — which is the
...
consequence is that CONVENTIONS §7's _"contrast ≥ 4.5:1 in both themes"_ is **unverifiable today**,
...
is _inconclusive_, not proof the site is down — but ten facts cite it and it is worth a manual
...
Every harness prints its own blind spots on every run. Consolidated, because _"an unstated
exemption reads as a clean pass"_:
...
- **INP is synthetic**: clicks on the first few interactive elements, reported as the maximum
  ...
  invisible.
- No Lighthouse score, so §7's _"Lighthouse a11y ≥ 95"_ is **not checked**. `lighthouse` is not a
...
technically present, or whether a focus ring is _visible enough_. No screen reader was driven.
...
change; per-host parsers are not re-run, so it reports _"this page moved"_, never _"the price went
from X to Y"_; `robots_disallowed` sources are never fetched, deliberately.
...
| 9   | **W3-C / owner**  | While `forcedTheme="light"` stands, _"contrast ≥ 4.5:1 in both themes"_ is unverifiable and half of every layout run is a duplicate (§4.5). If light-only is permanent, drop the theme axis from the harness and the budget; if it is temporary, say so, and I will leave the axis in place.                                                                                                                    |
...
1. **Budgets fail the run; they do not warn.** `tasks/W4-B`: _"A budget that only warns is a
   suggestion."_ Every one of the three harness bugs in §5 was found because something failed

...
`ConflictPopover` is built on and the conflict popover *is* the product; GSAP is W1-D's `Reveal`;
React is React.
...
- **W0-A — `app/not-found.tsx` and `app/global-error.tsx`, 2 lines each.** I edited files I do not
  ...
  Both files document *why* the plain anchor is right — a full navigation is the correct recovery
...
- `[GAP]` **LCP / CLS / INP NOT MEASURED.** No throttled run, no Lighthouse. `qa:perf` does not
  exist (W4-B). What *is* measured is the JS payload (§2) and that the `<h1>` is server-rendered

...
test.describe("evidence — the non-negotiables", () => {
  test("1 · every figure on the landing page carries a provenance affordance", async ({
    page,
  }) => {
    ...
    const values = page.locator(
      "[data-slot='provenance-value'], [data-provenance]"
    )
    ...
    const withoutAffordance = await values.evaluateAll(
      (nodes) =>
        nodes.filter((n) => {
          const el = n as HTMLElement
          const hasSource =
            el.querySelector("a[href^='http'], [data-slot='source-chip']") !==
            null
          const hasConfidence =
    ...
    expect(
      withoutAffordance,
      "figures rendered with no source and no confidence marker"
    ).toBe(0)
  })
  ...
  test("2 · F-002 shows all four competing prices and does not convert the USD one", async ({
    ...
    for (const figure of ["112.000", "185.000", "220.000", "239.171"]) {
      await expect(body, `competing price ${figure} is missing`).toContainText(
        figure
      )
    }
    ...
    expect(text, "the USD figure lost its currency").toMatch(
      /239[.,]171\s*(\$|USD)/
    )
    expect(text, "a converted USD→EUR figure appeared").not.toMatch(
      /239[.,]171\s*€/
    )
  })
  ...
  test("3 · a gap renders an em dash and never a zero", async ({ page }) => {
    ...
    const gaps = page.locator(
      "[data-confidence='gap'][data-slot='provenance-value']"
    )
    const count = await gaps.count()
    expect(
      count,
      "no gap facts on a page that should have some"
    ).toBeGreaterThan(0)

    for (let i = 0; i < count; i += 1) {
      const text = (await gaps.nth(i).innerText()).trim()
      expect(text, `gap rendered as ${JSON.stringify(text)}`).toContain("—")
      expect(text, `gap rendered a zero: ${JSON.stringify(text)}`).not.toMatch(
        /(^|\D)0(\D|$)/
      )
    }
  })
  ...
  test("4 · modelled records are distinguishable in the list, not only on a detail view", async ({
    ...
    expect(count, "no records carrying a data-quality marker").toBeGreaterThan(
      0
    )

    const marked = await blocks.evaluateAll(
      (nodes) =>
        nodes.filter((n) => {
          const el = n as HTMLElement
          const quality =
            el.getAttribute("data-quality") ??
            el.getAttribute("data-data-quality")
    ...
    expect(
      marked,
      "modelled records carry no visible mark in the list"
    ).toBeGreaterThan(0)
  })
  ...
  test("5 · a stale listing shows its badge beside the price, not in a footnote", async ({
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const figure = (
        await priceCell.locator("[data-numeric]").first().innerText()
      ).trim()
      const cell = (await priceCell.innerText()).trim()

      expect(
        figure.length,
        `stale row ${i} price cell holds no figure`
      ).toBeGreaterThan(0)
      expect(
  ...
  test("6 · the cockpit's counts match the dataset's own counts", async ({
    page,
    context,
  }) => {
    await loginAs(context, "manager")
    ...
    if (totals !== null) {
      expect(text, "656 is stated without its 25 + 631 composition").toMatch(
        /25|631/
      )
    }
  ...
  test("7 · no cross-platform review average appears anywhere", async ({
    page,
  }) => {
    await freezeMotion(page)
    await visit(page, localised("/hotel"))
    ...
    expect(text, "a combined score appeared").not.toMatch(
      /\b5[.,]6\d?\s*\/\s*10/
    )
    expect(text, "a combined score appeared").not.toMatch(
      /\b2[.,]8\d?\s*\/\s*5/

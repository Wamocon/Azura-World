...
test.describe("open redirect", () => {
  ...
  for (const next of HOSTILE) {
    test(`?next=${next} cannot leave the origin`, async ({ page }) => {
      await page.goto(
        `${localised("/login")}?next=${encodeURIComponent(next)}`,
        {
          waitUntil: "domcontentloaded",
        }
      )
      const url = new URL(page.url())
      expect(url.hostname, `left the origin for ${url.hostname}`).toBe(
        "127.0.0.1"
      )
    })
  }
...
test.describe("role elevation", () => {
  test("an unknown access-profile cookie resolves down, never up", async ({
    page,
    context,
  }) => {
    ...
    for (const hostile of HOSTILE) {
      ...
      expect(
        body,
        `cookie ${JSON.stringify(hostile)} produced an admin surface`
      ).not.toContain("administrator")
    }
  })

  test("a low role cannot reach the evidence cockpit's content", async ({
    page,
    context,
  }) => {
...
test.describe("untrusted content", () => {
  test("harvested review text renders as text, never as markup", async ({
    page,
  }) => {
    ...
    expect(unnonced, "an inline script without a nonce reached the page").toBe(
      0
    )
...
test.describe("the seed is never presented as live", () => {
  for (const role of ["admin", "manager"] as const) {
    test(`${role}: a surface serving seed data says so`, async ({
      page,
      context,
    }) => {
...
test.describe("no role sees a server error", () => {
  test("every role loads the dashboard home without a 5xx", async ({
    page,
    context,
  }) => {

  ...
  },
  {
    ...
    operations: [
      ...
      },
      {
        ...
        description:
          "Not implemented, and not implementable as described: `public.findings` has no status column. It holds id, severity, area, field_path, message, resolution, resolved_to and the two timestamps — so there is no field for the open/acknowledged/resolved/dismissed value this operation accepts, and nothing to write it to. This is a schema gap, not a missing repository function; closing it needs a migration that decides what a finding's lifecycle actually is. Until then the route validates, authorises, and reports 503 rather than accepting a judgement it would silently discard.",
        tag: "Evidence",
  ...
  // -- Sales ----------------------------------------------------------------
  {
    ...
    operations: [
      ...
      },
      {
        ...
        description:
          "Records an enquiry. A lead arrives unassigned and `new`; the payload carries no `status`, `assignedTo` or `score`, so a caller cannot route a lead to themselves or file one as already won. Row scope is `leads_staff_write` — administrator, or manager within their own company.",
        tag: "Sales",
  ...
  },
  {
    ...
    operations: [
      ...
      },
      {
        ...
        description:
          "Moves one entry to another stage. `expectedVersion` is not optional: a move without it is a last-writer-wins overwrite, and on a shared funnel that means two people move the same deal for different reasons and one reason vanishes. A mismatch is 409. `previous_stage` and `entered_stage_at` are maintained by the `track_pipeline_stage_change` trigger, not by the request, so the board's \"days in stage\" cannot disagree with the move that caused it.",
        tag: "Sales",
  ...
  // -- Governance -----------------------------------------------------------
  {
    ...
    operations: [
      ...
      },
      {
        ...
        summary: "Change a person's role or active state.",
        description:
          "The highest-authority mutation in this system. Two guards apply, both in Postgres: the last active administrator of a company cannot be demoted or deactivated (409), and a self-elevation is recorded in audit_events with metadata.selfElevation = true rather than refused. expectedVersion is compared against profiles.version.",
        tag: "Governance",
    ...
    operations: [
      {
        ...
        description:
          "The one write endpoint backed by real repository mutations. The body is a discriminated union on `command`, and each command declares its own permission, which is checked in addition to the endpoint's. Supported: `ticket.appendEvent`, `ticket.updateStatus`, `ledger.reverseEntry`, `vendorInvoice.settle`, `notification.markRead`. Commands carrying `expectedVersion` are optimistically concurrent: a mismatch is 409, never a silent overwrite. Every outcome writes an audit row.",
        tag: "Commands",
        permission: "dashboard:view",
        rateLimit: WRITE_LIMIT,

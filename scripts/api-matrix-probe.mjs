...
if (MODE !== "dev" && MODE !== "prod") {
  console.error(
    `AZURA_API_MODE must be "dev" or "prod", got ${JSON.stringify(MODE)}`,
  );
  process.exit(2);
}
...
const FIXTURES = {
  ...
  createProfile: [
    {
      email: "probe.profile@example.com",
      fullName: "Probe Profile",
      role: "tenant",
    },
    "createProfileSchema",
  ],
  ...
  executeCommand: [
    {
      command: "ticket.updateStatus",
      ticketId: UUID,
      expectedVersion: 1,
      toStatus: ticketStatuses[0],
    },
    "commandSchema",
  ],
  submitPublicReport: [
    {
      location: "Block B3, floor 4",
      description: "Probed by the W2-B route matrix.",
    },
    "publicReportSchema",
  ],
};
...
function finish(test, c, checked) {
  ...
  const more =
    c.failures.length > 10 ? `\n        (+${c.failures.length - 10} more)` : "";
  record(
    test,
    "fail",
    `${c.failures.length} of ${checked} case(s):\n        ${shown}${more}`,
  );
}
...
async function call(operation, { role, body, headers = {}, rawBody } = {}) {
  ...
  if (role !== undefined)
    init.headers["cookie"] = `access_profile_role=${role}`;
...
const operations = flattenOperations().filter(
  (op) => op.external === undefined,
);
...

{
  ...
  for (const op of mutating) {
    ...
    if (fixture === undefined) {
      c.fail(
        `${op.operationId}: declares requestSchema ${op.requestSchema} and has no fixture`,
      );
      continue;
    }
    ...
    if (schemaName !== op.requestSchema) {
      c.fail(
        `${op.operationId}: fixture targets ${schemaName}, manifest declares ${op.requestSchema}`,
      );
      continue;
    }
    ...
    if (schema === undefined) {
      c.fail(
        `${op.operationId}: lib/validation/schemas.ts exports no ${schemaName}`,
      );
      continue;
    }
    ...
    if (!parsed.success) {
      const issues = parsed.error.issues
        ...
        .join("; ");
      c.fail(
        `${op.operationId}: fixture is not valid against ${schemaName} — ${issues}`,
      );
    }
  }
  ...
  for (const op of guarded) {
    ...
    for (const role of roles) {
      ...
      if (!holds) {
        ...
        if (status !== 403) {
          c.fail(
            `${op.method} ${op.path} as ${role} (lacks ${op.permission}) → ${status}, expected 403`,
          );
        }
      } else if (status === 403) {
        c.fail(
          `${op.method} ${op.path} as ${role} (HOLDS ${op.permission}) → 403`,
        );
      } else if (status === 422 && body !== undefined) {
        c.fail(
          `${op.method} ${op.path} as ${role} → 422 with a schema-valid fixture; the fixture or the schema has drifted`,
        );
      }
    }
  }
...
if (MODE !== "prod") {
  ...
  for (const op of guarded) {
    ...
    if (status !== 401) {
      c.fail(
        `${op.method} ${op.path} unauthenticated → ${status}, expected 401`,
      );
    }
  }
  ...
  for (const op of guarded) {
    ...
    if (status !== 401) {
      b.fail(
        `${op.method} ${op.path} with access_profile_role=admin → ${status}, expected 401`,
      );
    }
  }
  finish(
    "2b · the access-profile cookie buys nothing in a production build",
    b,
    attempted,
  );
}
...
if (MODE !== "dev") {
  skip(
    "3 · writes return 503 with Supabase unconfigured",
    "needs a permitted role; run in dev mode.",
  );
} else {
  const c = collector();
  let checked = 0;
  for (const op of mutating) {
    ...
    if (holder === undefined) {
      c.fail(
        `${op.operationId}: no role holds ${op.permission}, so the write path is unreachable`,
      );
      continue;
    }
    const fixture = FIXTURES[op.operationId];
    ...
    if (status >= 200 && status < 300) {
      c.fail(
        `${op.method} ${op.path} as ${holder} → ${status}. A 2xx with no data plane is a fake write success.`,
      );
  ...
  for (const op of mutating) {
    for (const raw of ["{not json", '{"a":', "[]", "null"]) {
      ...
      if (status !== 422 && status !== 400 && status !== 403) {
        c.fail(
          `${op.method} ${op.path} with ${JSON.stringify(raw)} → ${status}, expected 422`,
        );
        continue;
      }
      if (status === 422 || status === 400) {
        ...
        try {
          ...
          c.fail(
            `${op.method} ${op.path} with ${JSON.stringify(raw)} → body is not JSON`,
          );
          continue;
        }
        if (parsed?.error?.code === undefined) {
          c.fail(
            `${op.method} ${op.path} with ${JSON.stringify(raw)} → no typed error code`,
          );
        }
      }
    }
...

{
  ...
  for (const op of mutating) {
    ...
    const field = Object.keys(body).find(
      (k) => typeof body[k] === "string" && k !== "command",
    );
    ...
    if (status !== 422) {
      c.fail(
        `${op.method} ${op.path} with a 5000-char ${field} → ${status}, expected 422`,
      );
      continue;
    }
    if (!text.includes(field)) {
      c.fail(
        `${op.method} ${op.path} 422 body does not name the offending field ${field}`,
      );
    }
  }
...

{
  ...
  if (idempotent.length === 0) {
    skip(
      ...
      "no operation in the manifest declares `idempotent: true`",
    );
  } else {
    const c = collector();
    let checked = 0;
    ...
    for (const op of idempotent) {
      ...
      const first = await call(op, {
        role,
        body,
        headers: { "idempotency-key": key },
      });
      const replay = await call(op, {
        role,
        body,
        headers: { "idempotency-key": key },
      });
      ...
      if (!stored) {
        notStored.push(
          `${op.method} ${op.path} (first response ${first.status})`,
        );
      ...
      if (replay.headers.get("idempotency-replayed") !== "true") {
        c.fail(
          `${op.method} ${op.path}: replay carried no Idempotency-Replayed header`,
        );
      }
      ...
      if (conflicting.status !== 409) {
        c.fail(
          `${op.method} ${op.path}: same key + different body → ${conflicting.status}, expected 409`,
        );
      }
    }
...
{
  const target = publicOps.find(
    (op) =>
      op.rateLimit !== undefined && FIXTURES[op.operationId] !== undefined,
  );
  if (target === undefined) {
    skip(
      "7 · rate limit returns 429 with Retry-After",
      "no public operation has both a rate limit and a fixture",
    );
  } else {
    const c = collector();
    ...
    for (let i = 0; i < budget + 3; i += 1) {
      const response = await call(target, {
        body: { ...body, description: `Burst probe ${i}.` },
      });
    ...
    if (limited === null) {
      c.fail(
        `${target.method} ${target.path}: ${budget + 3} requests against a limit of ${budget} never produced a 429`,
      );
    } else {
      const retryAfter = limited.headers.get("retry-after");
      if (retryAfter === null || Number.isNaN(Number(retryAfter))) {
        c.fail(
          `${target.method} ${target.path}: 429 carried Retry-After ${JSON.stringify(retryAfter)}`,
        );
      }
    }
  ...
  for (const capture of capturedBodies) {
    for (const [label, pattern] of needles) {
      if (pattern.test(capture.text)) {
        c.fail(
          `${capture.op} (${capture.status}) leaks ${label}: ${capture.text.slice(0, 160)}`,
        );
      }
    }
  }
  finish(
    "8 · no response body leaks postgres, PGRST, a stack frame or a path",
    c,
...
for (const r of results) {
  const mark =
    r.state === "pass" ? "PASS" : r.state === "skip" ? "SKIP" : "FAIL";

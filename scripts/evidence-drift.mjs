...
import { APP_DIR, createReporter, parseArgs, reportBlindSpots, resultDir, writeJson } from "./qa-lib.mjs"
...
function loadDataset() {
  ...
  return { harvest: read("harvest"), project: read("project"), hotel: read("hotel"), reviews: read("reviews") }
}
...
async function fetchOnce(url) {
  ...
  try {
    ...
    const tls = /certificate|TLS|SSL|UNABLE_TO_VERIFY|SELF_SIGNED/i.test(message)
    return { status: tls ? "tls_inconclusive" : "unreachable", error: message.slice(0, 160) }
  } finally {
    clearTimeout(timer)
  }
}
...
async function main() {
  ...
  const candidates = dataset.harvest
    ...
    .map((entry) => ({ ...entry, citedBy: factsCiting(dataset, entry.url).length }))
    .sort((a, b) => a.tier - b.tier || b.citedBy - a.citedBy)
    .slice(0, SAMPLE)

  const disallowed = dataset.harvest.filter((e) => String(e.status) === "robots_disallowed")
  console.log(`\n  ${dataset.harvest.length} harvested URLs · sampling ${candidates.length}`)
  console.log(`  ${disallowed.length} never fetched (robots.txt) and never will be by this harness`)
  ...
  for (const entry of candidates) {
    ...
    if (result.status === "tls_inconclusive") {
      ...
      reporter.note(`${entry.publisher}: TLS failed under Node — inconclusive, not a drift signal (see F-012)`)
    } else if (result.status === "unreachable") {
      verdict = "became-unreachable"
      findings.push({ ...record, verdict })
      console.log(`  DRIFT  ${entry.publisher} — now unreachable (was ${entry.status}); ${cites.length} fact(s) cite it`)
    } else if (typeof result.status === "number" && result.status >= 400) {
      verdict = "became-unreachable"
      findings.push({ ...record, verdict })
      console.log(`  DRIFT  ${entry.publisher} — HTTP ${result.status} (was ${entry.status}); ${cites.length} fact(s) cite it`)
    } else {
  ...
  reporter.check(
    ...
    drifted.length === 0,
    drifted.length === 0 ? `${candidates.length} sampled` : `${drifted.length} drifted`,
  )

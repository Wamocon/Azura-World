...
async function build() {
  ...
  if (!existsSync(assetsFile))
    throw new Error(
      "sources/media/assets.json missing — run node scripts/harvest-media.mjs",
    );
  ...
  const lqipById = existsSync(lqipFile)
    ? (JSON.parse(await readFile(lqipFile, "utf8")).lqip ?? {})
    : {};
  ...
  lines.push("");
  lines.push(
    `export const MEDIA_MANIFEST_GENERATED_AT = ${s(stats.generatedAt)} as const`,
  );
  lines.push("");
  lines.push(
  ...
  for (const r of rows) {
    ...
    lines.push(
      `    formats: { avif: [${f.avif.map(s).join(", ")}], webp: [${f.webp.map(s).join(", ")}], jpeg: [${f.jpeg.map(s).join(", ")}] },`,
    );
    ...
    lines.push(
      `    aspectRatio: ${r.encoded?.aspect ?? (a.width && a.height ? Number((a.width / a.height).toFixed(4)) : "null")},`,
    );
    ...
    lines.push(
      `    duplicateSourceHosts: [${(a.carriedByHosts ?? [])
        .filter((h) => h !== a.sourceHost)
        .map(s)
        .join(", ")}],`,
    );
  ...
  for (const [i, v] of (videoRefs ?? []).entries()) {
    ...
    const platform =
      v.platform ??
      (/youtu/.test(host) ? "youtube" : /vimeo/.test(host) ? "vimeo" : host);
    lines.push(`  {`);
...
async function run() {
  ...
  if (check) {
    ...
    const norm = (t) =>
      t
        .replace(/Generated : .*/g, "")
        .replace(/MEDIA_MANIFEST_GENERATED_AT = "[^"]*"/g, "");
    const same = norm(current) === norm(text);
    console.log(
      same
        ? "media-manifest.ts is current"
        : "media-manifest.ts is STALE — re-run node scripts/media-manifest.mjs",
    );
  ...
  console.log(
    `  wrote ${path.relative(REPO, OUT).replace(/\\/g, "/")} — ${text.split("\n").length} lines, ${(Buffer.byteLength(text) / 1024).toFixed(1)} KB`,
  );
  console.log(`\n── Assets by category ──`);
  for (const [k, v] of Object.entries(stats.byCategory).sort(
    (a, b) => b[1] - a[1],
  ))
    console.log(`  ${k.padEnd(10)} ${v}`);
  console.log(`\n── Assets by subject ──`);
  for (const [k, v] of Object.entries(stats.bySubject).sort(
    (a, b) => b[1] - a[1],
  ))
    console.log(`  ${k.padEnd(10)} ${v}`);
  console.log(`\n── Rights posture (usage) ──`);
  for (const k of ["internal_only", "unknown", "attributed_display"])
    console.log(`  ${k.padEnd(19)} ${stats.usage[k] ?? 0}`);
  console.log(`\n  video references: ${stats.videoRefs} (none rehosted)`);
  if (stats.skippedNoEncode)
    console.log(
      `  assets with no encoded variants: ${stats.skippedNoEncode} (PDF/vector or encode skipped — see sources/media/encoded.json)`,
    );
}

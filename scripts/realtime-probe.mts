...
modeCase(
  ...
  { online: true, supabaseConfigured: true, realtimeStatus: "subscribed", source: "supabase" },
  "realtime"
)
modeCase(
  "[2] subscription errored ⟹ polling",
  { online: true, supabaseConfigured: true, realtimeStatus: "error", source: "supabase" },
  "polling"
)
modeCase(
  "[2] subscription closed ⟹ polling",
  { online: true, supabaseConfigured: true, realtimeStatus: "closed", source: "supabase" },
  "polling"
)
modeCase(
  "[2] still subscribing ⟹ polling, NOT realtime",
  { online: true, supabaseConfigured: true, realtimeStatus: "subscribing", source: "supabase" },
  "polling"
)
modeCase(
  "[3] Supabase unconfigured ⟹ static",
  { online: true, supabaseConfigured: false, realtimeStatus: "idle", source: null },
  "static"
)
modeCase(
  "[3] configured but the repository returned local-seed ⟹ static",
  { online: true, supabaseConfigured: true, realtimeStatus: "subscribed", source: "local-seed" },
  "static",
)
modeCase(
  "[4] offline outranks everything",
  { online: false, supabaseConfigured: true, realtimeStatus: "subscribed", source: "supabase" },
  "offline"
)
modeCase(
  "[4] offline outranks static",
  { online: false, supabaseConfigured: false, realtimeStatus: "idle", source: null },
  "offline"
)

section("[DoD 3] static and offline must NOT poll — zero timers, zero requests")
check("shouldPoll(static) is false", !shouldPoll("static"))
check("shouldPoll(offline) is false", !shouldPoll("offline"))
check("shouldPoll(realtime) is false", !shouldPoll("realtime"), "realtime is pushed, not pulled")
...
check(
  ...
  sequence.every((value, index) => index === 0 || value >= (sequence[index - 1] ?? 0))
)
check(
...

{
  ...
  scheduled?.()
  check("the quiet window produced exactly ONE run", runs === 1, `${runs} run(s)`)
...

{
  ...
  coalescer.cancel()
  scheduled?.()
  check("[DoD 6] cancel() drops pending work — nothing runs after unmount", runs === 0)
...
{
  const snapshot = [{ id: "a", n: 1 }, { id: "b", n: 2 }]
  const optimistic = [{ id: "a", n: 1 }, { id: "b", n: 2 }, { id: "c", n: 3 }]
  ...
  check(
    ...
    JSON.stringify(
      resolveMutationOutcome({ snapshot, optimistic, succeeded: true, rollbackOnError: true })
    ) === JSON.stringify(optimistic)
  )
  check(
    "rollbackOnError=false keeps the optimistic state on failure",
    JSON.stringify(
      resolveMutationOutcome({ snapshot, optimistic, succeeded: false, rollbackOnError: false })
    ) === JSON.stringify(optimistic)
  )
}
...
for (const path of ["/de/dashboards-public", "/de/dashboardx", "/de/hotel/dashboard-tour"]) {
  check(
...
section("[DoD 8] The service worker's copy of the policy cannot drift")
{
  ...
  check(
    ...
    sw.indexOf("isProtectedPath(url.pathname)") < sw.indexOf("STATIC_PREFIXES.some"),
...
check("relativeAge seconds", relativeAge("2026-07-27T12:00:00.000Z", base + 12_000) === "12 s")
check("relativeAge minutes", relativeAge("2026-07-27T12:00:00.000Z", base + 240_000) === "4 min")
check("relativeAge hours", relativeAge("2026-07-27T12:00:00.000Z", base + 7_200_000) === "2 h")

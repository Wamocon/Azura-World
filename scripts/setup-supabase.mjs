...
const MB = 1024 * 1024
const mb = (n) => (typeof n === "number" ? `${Math.round(n / MB)} MB` : String(n ?? "unset"))
...
const BUCKETS = [
  {
    ...
    purpose: "contracts, invoices, unit documents — read via short-TTL signed URLs",
    fileSizeLimit: 25 * MB,
    allowedMimeTypes: [
      "application/pdf",
      "image/jpeg",
  ...
  },
  {
    ...
    allowedMimeTypes: ["text/html", "text/plain", "application/json", "image/*"],
  },
]
...
const why = (r) =>
  r.json && typeof r.json.message === "string" ? r.json.message : `HTTP ${r.status}`
...
try {
  ...
  record("Storage service reachable", "PASS", `${list.length} bucket(s) present`)
} catch (err) {
  record(
...
if (DRY_RUN) {
  ...
  console.log("Review the WARN lines above before treating storage as configured.")

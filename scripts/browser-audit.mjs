    ...
    for (const locale of LOCALES) {
      for (const route of ROUTES) {
        ...
        page.on("requestfailed", (request) => {
          failedRequests.push(`${request.method()} ${request.url().slice(0, 120)} — ${request.failure()?.errorText ?? "?"}`)
        })
        page.on("response", (response) => {
          if (response.status() >= 400) {
            failedRequests.push(`HTTP ${response.status()} ${response.url().slice(0, 120)}`)
          }
        })
        ...
        if (bad > 0) {
          findings.push({ locale, route: route.name, url, finalUrl, status, errors, failedRequests, warnings })
        }
        reporter.check(
        ...
        for (const failure of failedRequests.slice(0, 3)) console.log(`         ${failure}`)

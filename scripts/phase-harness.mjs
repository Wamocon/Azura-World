...
import { ROOT, createReporter, parseArgs, resultDir, writeJson } from "./qa-lib.mjs"
 ...
 */
const GATES = {
  typecheck: { cmd: ["pnpm", "--dir", "apps/web", "typecheck"], profiles: ["smoke", "full"] },
  lint: { cmd: ["pnpm", "--dir", "apps/web", "lint"], profiles: ["smoke", "full"] },
  ...
  i18n: { cmd: ["node", "scripts/check-i18n.mjs"], profiles: ["smoke", "full"] },
  ...
  drift: { cmd: ["node", "scripts/evidence-drift.mjs", "--report-only"], profiles: ["full"], required: false },
}
...
const WAVE_GATES = {
  ...
  3: ["typecheck", "lint", "build", "contracts", "evidence", "i18n", "csp", "browser"],
  4: ["typecheck", "lint", "build", "contracts", "evidence", "i18n", "csp", "layout", "perf", "a11y", "browser", "drift"],
  5: ["typecheck", "lint", "build", "contracts", "evidence", "i18n", "csp", "layout", "perf", "a11y", "browser", "drift"],
}
...
async function main() {
  if (WAVE === undefined || !(WAVE in WAVE_GATES)) {
    console.error(`usage: node scripts/phase-harness.mjs --wave <0-5> [--profile smoke|full]`)
    process.exit(2)
  }
  ...
  const names = WAVE_GATES[WAVE].filter((name) => GATES[name].profiles.includes(PROFILE))

  reporter.section(`Wave ${WAVE} · profile ${PROFILE} · ${names.length} gate(s)`)
  ...
  for (const result of results) {
    const mark = result.exitCode === 0 ? "ok  " : result.required ? "FAIL" : "warn"
    console.log(`  ${mark}  ${result.name.padEnd(12)} exit=${String(result.exitCode).padStart(3)}  ${(result.durationMs / 1000).toFixed(1)}s`)
  }
  console.log(`\n  report: ${path}`)

...
function check(id, severity, title, owner, fn) {
  const slot = results.push({ id, severity, title, owner, state: "pending", detail: null }) - 1
  ...
  const fail = (error) => {
    if (error instanceof SkipError) {
      skippedGates += 1
      results[slot] = { id, severity, title, owner, state: "skipped", detail: error.message }
    } else {
      results[slot] = {
        id,
        severity,
...
const loginActions = await tryImport("../apps/web/app/[locale]/login/actions.ts")

function need(mod, name) {
  if (mod.__failed) skip(`${name} could not be loaded outside a Next request scope: ${mod.__failed}`)
  return mod
}
...
check(
  ...
  () => {
    ...
    c.assert(allowedCount > 0, "the matrix produced no allowed pairs at all — the probe is not exercising anything")
...
check(
  ...
  () => {
    ...
    const c = collect()
    const cases = [
      { name: "no user", facts: { userId: null, userEmail: null, profileReadFailed: false, row: null }, maxRole: null },
      { name: "read failed", facts: { userId: "u", userEmail: null, profileReadFailed: true, row: null }, maxRole: "tenant" },
      { name: "no row", facts: { userId: "u", userEmail: null, profileReadFailed: false, row: null }, maxRole: "tenant" },
      { name: "suspended admin", facts: { userId: "u", userEmail: null, profileReadFailed: false, row: { role: "admin", is_active: false } }, maxRole: null },
      { name: "anonymised admin", facts: { userId: "u", userEmail: null, profileReadFailed: false, row: { role: "admin", is_active: true, anonymized_at: "2026-01-01T00:00:00Z" } }, maxRole: null },
      { name: "unknown role string", facts: { userId: "u", userEmail: null, profileReadFailed: false, row: { role: "superuser", is_active: true } }, maxRole: "tenant" },
      { name: "role injected as object", facts: { userId: "u", userEmail: null, profileReadFailed: false, row: { role: { toString: () => "admin" }, is_active: true } }, maxRole: "tenant" },
      { name: "roles[] claims admin, role says tenant", facts: { userId: "u", userEmail: null, profileReadFailed: false, row: { role: "tenant", roles: ["admin"], is_active: true } }, maxRole: "admin" },
    ]
    for (const { name, facts, maxRole } of cases) {
      const profile = authResolution.resolveSupabaseProfile(facts)
      if (maxRole === null) {
        c.assert(profile.authenticated === false, `"${name}" produced an authenticated profile (role ${profile.role})`)
        continue
      }
      c.assert(
...
check(
  ...
  () => {
    ...
    return "roles[] widens authority (['admin'] on a tenant yields " + JSON.stringify(list) + ") — so it needs the same write protection as profiles.role, which prevent_profile_privilege_escalation() does not give it"
  }
)
...
check(
  ...
  () => {
    ...
    const select = /\.from\("profiles"\)[\s\S]{0,400}?\.select\(\s*"([^"]+)"/.exec(authSource)
    if (select === null) skip("could not locate the profiles select() in lib/auth.ts")
    ...
    if (profilesDdl.length === 0) skip("no CREATE/ALTER TABLE public.profiles found in the migrations")

    const missing = columns.filter((col) => !new RegExp(`\\b${col}\\b`).test(profilesDdl))
...
check(
  ...
  () => {
    ...
    const dataPlanes = [
      {},
      { NEXT_PUBLIC_SUPABASE_URL: "https://x.supabase.co", NEXT_PUBLIC_SUPABASE_ANON_KEY: "k" },
    ]
...
check(
  ...
  () => {
    ...
    for (const value of hostile) {
      ...
      c.assert(
        contracts.roleLevel[role] <= contracts.roleLevel[policy.DEFAULT_ACCESS_PROFILE_ROLE],
  ...
  () => {
    ...
    return Promise.all(
      hostile.map(async (value) => {
        ...
        c.assert(
          out.startsWith("/") && !out.startsWith("//") && !out.includes("\\") && !/[ -]/.test(out),
...
check(
  ...
  () => {
    ...
    try {
      ...
      skip(`git ls-files failed: ${error instanceof Error ? error.message : String(error)}`)
    }
    const sourceish = /\.(ts|tsx|mts|mjs|js|jsx|json|sql|md|yml|yaml|css|py|sh)$/
    const offenders = []
...
check(
  ...
  () => {
    ...
    const usesLogOrRevList = /Scan history[\s\S]{0,600}?git (log|rev-list)/.test(ci)
...
check(
  ...
  () => {
    ...
    if (!existsSync(staticDir)) skip("no production build present — run `pnpm --dir apps/web build` first")
...
check(
  ...
  () => {
    ...
    const leaked = files.filter((f) => readFileSync(f, "utf8").includes(fingerprint))
...
check(
  ...
  () => {
    ...
    for (const page of pages) {
      ...
      const checks =
        /hasPermission\s*\(|profileCan\s*\(|requireProfile\s*\(|forbidden\s*\(|notFound\s*\(/.test(source)
      c.assert(
        checks,
  ...
  async () => {
    ...
    for (const role of ["tenant", "guest", "owner", "staff"]) {
      ...
      try {
        ...
        skip(`could not reach ${base}: ${error instanceof Error ? error.message : String(error)}`)
      }
...
check(
  ...
  () => {
    ...
    for (const testCase of cases) {
      const rendered = format.formatFactValue(testCase.value, testCase.format, testCase.locale)
    ...
    return c.result("formatter alters the value") + " — formatNumber() defaults to maximumFractionDigits: 0 and the \"number\" case never overrides it"
  }
)
...
check(
  "SEC-H02",
  "high",
  "A gap fact never renders as 0",
  "W1-D",
  () => {
    need(format, "components/evidence/format.ts")
    const c = collect()
    for (const f of ["number", "area", "money", "metres", "kilometres", "percent", "stars", "text", "date"]) {
...
check(
  ...
  () => {
    ...
    for (const block of blocks) {
      ...
      const carried = competing === null ? 0 : (competing[1].match(/\{\s*value:/g) ?? []).length
      c.assert(
        carried >= claimed,
...
check(
  ...
  () => {
    ...
    for (const block of blocks) {
      ...
      const message = /message:\s*\n?\s*"((?:[^"\\]|\\.)*)"/.exec(block)
      if (message === null) continue
      const hasRatio = /\d+([.,]\d+)?\s*x\s+range|\d+([.,]\d+)?-?fache/i.test(message[1])
      if (!hasRatio) continue
      const currencies = new Set((block.match(/currency:\s*"([A-Z]{3})"/g) ?? []).map((m) => m.slice(-4, -1)))
      c.assert(
        currencies.size <= 1,
...
check(
  ...
  () => {
    ...
    for (const file of sources) {
      ...
      const pattern = /(?:const|let)\s+(\w+)\s*=\s*createServiceRoleClient\(\)/g
      let match
...
for (const r of results) {
  const mark = r.state === "clean" ? "PASS" : r.state === "skipped" ? "SKIP" : "OPEN"
...
if (blocking.length > 0) {
  ...
  for (const r of blocking) console.log(`  ${r.id}  [${r.severity}]  ${r.title}  → ${r.owner}`)
  console.log(bar)
  process.exit(1)
}
...
if (skippedGates > 0) {
  console.log("")
  console.log("No Critical or High reproduced, but a gate could not run. Exiting 2.")
  console.log(bar)
  process.exit(2)
}

...
const check = (label, ok, detail = "") => {
  if (ok) {
    ...
    failures += 1
    console.log(`  ${c("31", "FAIL")}  ${label} — ${detail || "condition was false"}`)
  }
}
...
async function launchChromium() {
  try {
    ...
    console.log(`  ${c("33", "INFO")}  falling back to ${candidate} (pinned build absent)`)
    return { browser: await chromium.launch({ executablePath }), revision: candidate }
  }
}

const { browser, revision } = await launchChromium()

async function open(path, { width = 1280, height = 1400, colorScheme = "light", reducedMotion = "no-preference" } = {}) {
...
for (const [label, pattern] of priceProbes) {
  check(label, pattern.test(body), pattern.test(body) ? "present as text" : "MISSING")
}
...
const publishers = ["Haspo Realty", "Seaside Alanya", "Housearch", "Capital Estate", "Alanya-Home"]
...
check("one rail per currency", new Set(railCurrencies).size === rails.length, railCurrencies.join(", "))
check(
...
const ladderText = await de.page.innerText('[data-slot="price-conflict-ladder"]')
check(
...
const staleRows = await de.page.$$('[data-slot="observation-row"][data-stale="true"]')
...
const staleTicks = await de.page.$$('[data-slot="ladder-tick"][data-stale="true"]')
check("stale ticks are marked on the ladder too", staleTicks.length > 0, `${staleTicks.length} ticks`)
const tickShape = await de.page.evaluate(() => {
  const stale = document.querySelector('[data-slot="ladder-tick"][data-stale="true"]')
  const fresh = document.querySelector('[data-slot="ladder-tick"][data-stale="false"]')
  if (!stale || !fresh) return null
  const a = getComputedStyle(stale)
  const b = getComputedStyle(fresh)
  return { staleStyle: a.borderStyle, staleWidth: a.width, freshWidth: b.width }
})
check(
  "stale is a SHAPE difference, not only a colour difference",
  tickShape !== null && tickShape.staleStyle === "dashed" && tickShape.staleWidth !== tickShape.freshWidth,
  tickShape ? `stale ${tickShape.staleWidth}/${tickShape.staleStyle} vs fresh ${tickShape.freshWidth}` : "no ticks"
)
...
const railTicks = await de.page.evaluate(() =>
  Array.from(document.querySelectorAll('[data-slot="ladder-rail"]')).map((rail) => ({
    currency: rail.getAttribute("data-currency"),
    ticks: Array.from(rail.querySelectorAll('[data-slot="ladder-tick"]')).map((el) => ({
...
check("ticks carry a positioned left offset", allTicks.every((t) => Number.isFinite(t.left)))
check(
...
for (const rail of railTicks) {
  ...
  const monotonic = sorted.every((t, i) => i === 0 || t.left >= sorted[i - 1].left)
  check(`${rail.currency}: a higher price sits further right`, monotonic, `${sorted.length} ticks`)
  ...
  const ratio = (sorted[sorted.length - 1]?.amount ?? 1) / (sorted[0]?.amount ?? 1)
  ...
  check(
    ...
    sorted.length < 2 ? "single observation" : `${lo}% → ${hi}% (${clustered ? "clustered" : "spanned"})`
  )
}
...
check(
  ...
  (await de.page.getAttribute('[data-slot="price-conflict-ladder"] [aria-hidden="true"]', "aria-hidden")) === "true",
  "the table below carries the same observations as text"
)
const tableRows = await de.page.$$('[data-slot="observation-row"]')
check("every observation exists as a table row", tableRows.length >= 15, `${tableRows.length} rows`)
const linkCount = await de.page.$$eval('[data-slot="observation-row"] a[href^="http"]', (a) => a.length)
check("every row offers a route back to its source", linkCount >= tableRows.length, `${linkCount} links / ${tableRows.length} rows`)
...
check("all tap targets ≥ 24px high", smallTargets.length === 0, smallTargets.length === 0 ? "0 violations" : JSON.stringify(smallTargets.slice(0, 4)))

await de.page.screenshot({ path: new URL("evidence-f002-de-light.png", OUT).pathname.slice(1), fullPage: true })

const cspViolations = await de.page.evaluate(() => window.__cspViolations ?? [])
check("no CSP violations on the page", cspViolations.length === 0, cspViolations.slice(0, 3).join(", ") || "0")
...
await narrow.page.screenshot({ path: new URL("evidence-f002-de-320.png", OUT).pathname.slice(1), fullPage: true })
await narrow.context.close()

// reduced motion — a complete, static page
const reduced = await open("/de/dashboard/evidence", { reducedMotion: "reduce" })

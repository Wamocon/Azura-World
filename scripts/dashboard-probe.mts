...
check(
  ...
  dashboardRoutes.every((r) => r.href === "/dashboard" || r.href.startsWith("/dashboard/")),
  dashboardRoutes.filter((r) => !r.href.startsWith("/dashboard")).map((r) => r.href).join(", "),
)
...
console.log(`   matrix cells crossed: ${matrixCells} (${roles.length} roles × ${dashboardRoutes.length} routes)`)
check(
...
console.log("   nav size by role: " + navSizes.map(([r, n]) => `${r}=${n}`).join(" "))
...
check("normalize strips a trailing slash", normalizeDashboardPath("/dashboard/") === "/dashboard")
check("normalize adds a leading slash", normalizeDashboardPath("dashboard") === "/dashboard")

...
const TERRA = src(
  "https://terra-property.com/azura-world",
  "Terra",
  4,
  hash("e"),
);
...
expectEqual(
  "displayValue(well-formed gap) is null",
  displayValue(gapFact),
  null,
);
...
expectEqual(
  "tier 4 vs tier 1 → tier 1",
  tierWins(SEASIDE_1, OFFICIAL),
  OFFICIAL,
);
expectEqual(
...
expectEqual(
  "isSourcedFact(well-formed fact)",
  isSourcedFact(officialFact),
  true,
);

...
function check(label, passed, detail) {
  ...
  console.log(`  ${mark}  ${label}${detail === undefined ? "" : `  \x1b[2m— ${detail}\x1b[0m`}`)
}
...
try {
  ...
  check("manager receives 200", managerView.status === 200, `status ${managerView.status}`)
  check(
  ...
  for (const figure of ["112.000", "185.000", "220.000", "239.171"]) {
    check(`competing price ${figure} present`, managerView.body.includes(figure))
  }
  check(
    "the USD figure is not converted",
    /239[.,]171[^€]{0,40}(\$|USD)/.test(managerView.body) && !/239[.,]171\s*€/.test(managerView.body),
  )
  check(
  ...
  check("manager may export", exportManager.status === 200, `status ${exportManager.status}`)
  check(
    "served as a CSV download",
    (exportManager.headers.get("content-type") ?? "").includes("text/csv") &&
      (exportManager.headers.get("content-disposition") ?? "").includes("attachment"),
  ...
  for (const required of ["source_url", "source_publisher", "source_fetched_at", "snapshot_hash"]) {
    check(`provenance column "${required}"`, columns.includes(required))
  }
  check("price amount and currency are separate columns", columns.includes("price_amount") && columns.includes("price_currency"))
  check("no converted-currency column", !columns.some((c) => /_eur$|_in_eur|converted/i.test(c)))
  check("rows exported", rows.length > 0, `${rows.length} row(s)`)

  const withUrl = rows.filter((row) => row.includes("http")).length
  check("every row carries a source URL", withUrl === rows.length, `${withUrl}/${rows.length}`)

  const currencies = new Set(
    rows.map((row) => row.split(",")[columns.indexOf("price_currency")]).filter(Boolean),
  )
  check(
  ...
  check(
    ...
    (exportManager.headers.get("content-disposition") ?? "").includes("LOCAL-SEED") ||
  ...
  check("tenant is refused the export", exportTenant.status === 403, `status ${exportTenant.status}`)
  ...
  // -------------------------------------------------------------------------
  gap("Gap 3 — the permission matrix: manager reads, admin annotates, tenant 403")

  const adminView = await get("/de/dashboard/evidence", "admin")
  check("admin receives 200", adminView.status === 200, `status ${adminView.status}`)
  check("admin is offered the annotation form", adminView.body.includes('data-slot="annotation-form"'))
  check(
    "manager is NOT offered the annotation form",
    !managerView.body.includes('data-slot="annotation-form"'),
  )
  check("manager IS offered the export", managerView.body.includes('data-testid="evidence-export"'))
  ...
  for (const needle of ["Housearch", "239.171", "112.000", "Haspo", "Seaside"]) {
    check(
  ...
  const { roles } = await import("../apps/web/lib/contracts.ts").catch(() => ({ roles: null }))
  if (roles === null) {
    check("role sweep", true, "skipped — contracts.ts not loadable from plain node")
  }
  for (const role of ["accountant", "staff", "owner", "guest", "service_provider", "child_owner"]) {
    const view = await get("/de/dashboard/evidence", role)
    const leaked = ["Housearch", "239.171"].filter((n) => view.body.includes(n))
    check(`${role} receives no evidence`, leaked.length === 0, leaked.join(", ") || "clean")
  }
  ...
  check("the units list renders", units.status === 200, `status ${units.status}`)
  check("the split summary is present", units.body.includes("25") && units.body.includes("631"))
  check("the total is stated", units.body.includes("656"))

  const modelledMarks = (units.body.match(/data-quality="modelled"/g) ?? []).length
  const portalMarks = (units.body.match(/data-quality="portal_listing"/g) ?? []).length
  check(

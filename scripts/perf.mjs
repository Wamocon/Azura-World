...
function analyseBundle(reporter) {
  ...
  if (!existsSync(chunkDir)) {
    reporter.check(
      "built chunks found",
      false,
      `${chunkDir} missing — run \`pnpm --dir apps/web build\``,
    );
    return null;
  }
  ...
  console.log(
    `  chunks: ${result.chunkCount}   3D-bearing: ${threeFiles.length}`,
  );
  for (const f of result.threeFiles)
    console.log(`    3D   ${String(f.gzipKb).padStart(7)} KB  ${f.file}`);
  for (const f of result.largestApp.slice(0, 4))
    console.log(`    app  ${String(f.gzipKb).padStart(7)} KB  ${f.file}`);
...
async function measure(browser, url, { throttle, warm }) {
  ...
  try {
    ...
    for (const target of targets.slice(0, 4)) {
      await target
        .click({ trial: false, timeout: 1500, noWaitAfter: true })
        .catch(() => {});
...
function aggregate(samples) {
  ...
  const withResources = samples.filter(
    (s) => Array.isArray(s.jsResources) && s.jsResources.length > 0,
  );
  out.jsResources =
    withResources.length === 0
      ? []
      : withResources.reduce((best, s) =>
          s.jsResources.length > best.jsResources.length ? s : best,
        ).jsResources;
  return out;
}
...
async function soak(browser, url, reporter, ms) {
  ...
  console.log(
    `  canvases: ${first.canvases} → ${last.canvases}   DOM nodes: ${first.nodes} → ${last.nodes}`,
  );
  if (growthMb === null) {
    reporter.note("performance.memory unavailable — heap growth NOT MEASURED");
  } else {
    console.log(
      `  JS heap: ${(first.heap / 1048576).toFixed(1)} MB → ${(last.heap / 1048576).toFixed(1)} MB  (${growthMb >= 0 ? "+" : ""}${growthMb} MB)`,
    );
    // A rendering loop that leaks a frame's worth of objects per frame grows
    // fast; 50 MB over a minute is well clear of GC noise.
    reporter.check(
      "no runaway heap growth over the soak",
      growthMb < 50,
      `${growthMb} MB`,
    );
  }
  reporter.check(
    "no console errors during the soak",
    errors.length === 0,
    errors.length === 0 ? "" : errors.slice(0, 3).join(" | "),
  );
  reporter.check(
    "no canvas leak",
    last.canvases <= Math.max(1, first.canvases),
...
async function main() {
  ...
  console.log(
    `  LCP ≤ ${BUDGETS.lcpMs}ms (throttled mobile) · CLS ≤ ${BUDGETS.cls} · INP ≤ ${BUDGETS.inpMs}ms`,
  );
  console.log(
    `  landing JS ≤ ${BUDGETS.landingJsKb} KB gz (excluding 3D) · lazy 3D chunk ≤ ${BUDGETS.chunk3dKb} KB gz`,
  );
  ...
  try {
    ...
    for (const [phase, throttle] of [
      ["Phase 1 — desktop", false],
      ["Phase 2 — throttled mobile (Slow 4G, 4× CPU)", true],
    ]) {
    ...
    for (const route of routes) {
      ...
      reporter.check(
        `${route.name} LCP ≤ ${BUDGETS.lcpMs}ms (mobile, cold)`,
        cold.lcpMs <= BUDGETS.lcpMs,
        `${cold.lcpMs}ms`,
      );
      ...
      const worstCls = clsSamples.reduce((worst, s) =>
        s.value.cls > worst.value.cls ? s : worst,
      );
      reporter.check(
      ...
      const inp = Math.max(
        cold.inpMs,
        results[`${route.name}:desktop:cold`].inpMs,
      );
      reporter.check(
        `${route.name} INP ≤ ${BUDGETS.inpMs}ms`,
        inp <= BUDGETS.inpMs,
        `${inp}ms (max observed)`,
      );
    }
    ...
    if (bundle !== null) {
      ...
      const threeNames = new Set(
        bundle.threeFiles.map((f) => f.file.split("/").pop()),
      );

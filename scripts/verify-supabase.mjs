...
record(
  ...
  REF ? "PASS" : "WARN",
  REF ?? "not set (CLI link will need it)",
);
record(
  "SUPABASE_DB_URL",
  DBURL ? "PASS" : "WARN",
  DBURL ? "set" : "not set (migrations will need it)",
);
...
for (const k of [
  ...
  record(
    k,
    v && v.length >= 32 ? "PASS" : "FAIL",
    v ? `len=${v.length}` : "not set",
  );
}
...
record(
  "URL is a supabase.co project URL",
  urlRef ? "PASS" : "FAIL",
  urlRef ?? URL_,
);
if (REF && urlRef) {
  record(
    "PROJECT_REF matches URL",
    REF === urlRef ? "PASS" : "FAIL",
    REF === urlRef ? REF : `${REF} vs ${urlRef}`,
  );
}
record(
...
if (DBURL && urlRef) {
  record(
    "DB_URL references the same project",
    DBURL.includes(urlRef) ? "PASS" : "WARN",
    DBURL.includes(urlRef)
      ? "ok"
      : "project ref not found in connection string",
  );
...
async function probe(label, path, key) {
  try {
    ...
    record(
      label,
      "FAIL",
      err.name === "TimeoutError" ? "timed out after 15s" : err.message,
    );
    return null;
  }
}
...
const anonRes = await probe(
  "REST reachable with anon key",
  "/rest/v1/__azura_probe__?select=*",
  ANON,
);
if (anonRes) {
  ...
  record(
    ...
    invalidKey
      ? "rejected — key wrong, rotated, or project paused"
      : "reached PostgREST",
  );
  if (VERBOSE) console.log(`      → ${body.slice(0, 160)}`);
}

const svcRes = await probe(
  "REST reachable with service-role key",
  "/rest/v1/",
  SVC,
);
if (svcRes) {
  record(
    "REST reachable with service-role key",
    svcRes.status < 500 ? "PASS" : "FAIL",
    `HTTP ${svcRes.status}`,
  );
  if (svcRes.status === 401)
    record("service-role key accepted", "FAIL", "401 — key rejected");
}
...
try {
  ...
  if (res.ok && VERBOSE) {
    ...
    console.log(
      `      → external providers: ${
        Object.keys(s.external ?? {})
          .filter((k) => s.external[k])
          .join(", ") || "none"
      }`,
    );
  }
    ...
    for (const want of [
      ...
      if (!b)
        record(
          `bucket "${want}"`,
          "WARN",
          "missing — run scripts/setup-supabase.mjs",
        );
      else
        record(
          `bucket "${want}"`,
          b.public ? "FAIL" : "PASS",
          b.public ? "PUBLIC — must be private" : "private",
        );
    }
  } else {
  ...
  if (res.ok) {
    ...
    if (tables.length === 0) {
      ...
      record(
        "Database state",
        "WARN",
        `not empty: ${tables.slice(0, 8).join(", ")}${tables.length > 8 ? "…" : ""}`,
      );
      console.log(
        "      \x1b[33mReview these before running migrations — W1-A assumes a clean project.\x1b[0m",
      );
    }
  } else {
...
if (!DBURL) {
  ...
  try {
    ...
    if (VERBOSE)
      console.log(
        `      → db=${rows[0].current_database} user=${rows[0].current_user}`,
      );
    ...
    for (const want of ["pgcrypto", "pg_trgm"]) {
      record(
        `extension ${want}`,
        ext.rows.some((r) => r.extname === want) ? "PASS" : "WARN",
        ext.rows.some((r) => r.extname === want)
          ? "installed"
          : "W1-A migration 00 will enable it",
      );
    }
    await client.end();
  } catch (err) {
    if (err.code === "ERR_MODULE_NOT_FOUND") {
      record(
        "Direct connection",
        "SKIP",
        "`pg` not installed — run after pnpm install",
      );
    } else {

...
const properNounData = JSON.parse(
  readFileSync(join(repoRoot, "apps", "web", "lib", "proper-nouns.json"), "utf8")
)
...
function loadCatalogue(locale) {
  ...
  try {
    ...
    error("0a", locale, "", `cannot read ${relative(repoRoot, file)}: ${cause.message}`)
    return null
  }
...
function translatableRemainder(value) {
  let remainder = value
  for (const term of [...properNounData.properNouns, ...properNounData.sharedTerms].sort(
...
for (const locale of LOCALES) {
  ...
  for (const [key, value] of flat) {
    ...
    // genuinely is the lowercase key word (`common.units.rooms: "rooms"`).
    {
      ...
      if (trimmed === key) {
        error("5", locale, key, "value equals its full key path — unfilled stub")
    ...
    if (locale !== DEFAULT_LOCALE) {
      ...
      if (baseValue !== undefined) {
        ...
        if (missing.length > 0 || extra.length > 0) {
          ...
          error("4", locale, key, `placeholder mismatch vs ${DEFAULT_LOCALE}: ${parts.join("; ")}`)
        }
      }
    }
    ...
    if (locale !== DEFAULT_LOCALE) {
      const baseValue = base.get(key)
      if (baseValue !== undefined && baseValue === value && !isAllowlisted(value)) {
...
function report() {
  ...
  for (const locale of LOCALES) {
    ...
    const marker = locale === DEFAULT_LOCALE ? " (default, source of truth)" : ""
    process.stdout.write(`  ${locale}.json  ${String(size).padStart(4)} keys${marker}\n`)
  }
  process.stdout.write("\n")
  ...
  if (warnings.length > 0) {
    ...
    for (const item of warnings) {
      process.stdout.write(`  [${item.rule}] ${item.locale}  ${item.key}\n        ${item.message}\n`)
    }
    process.stdout.write("\n")
  }
  ...
  if (errors.length === 0) {
    ...
    process.stdout.write(`PASS — 0 errors, ${warnings.length} warnings, ${parity}\n`)
    return
  }

  process.stdout.write(`ERRORS (${errors.length})\n`)
  for (const item of errors) {
    process.stdout.write(`  [rule ${item.rule}] ${item.locale}  ${item.key}\n        ${item.message}\n`)
  }

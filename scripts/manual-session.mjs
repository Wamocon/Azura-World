...
const playwright = await import(pathToFileURL(requireFromApp.resolve("@playwright/test")).href)
...
async function session(name, { role, locale = "de", width = 1440, height = 900, mobile = false }) {
  const context = await browser.newContext({
    viewport: { width, height },
    ...(mobile ? { isMobile: true, hasTouch: true, deviceScaleFactor: 3 } : {}),
    recordVideo: { dir: join(OUT, "video"), size: { width, height } },
    locale: locale === "de" ? "de-DE" : locale === "ru" ? "ru-RU" : locale === "tr" ? "tr-TR" : "en-GB",
  })
  await context.tracing.start({ screenshots: true, snapshots: true, sources: false })

  if (role !== undefined) {
    await context.addCookies([
      { name: "access_profile_role", value: role, domain: "127.0.0.1", path: "/", sameSite: "Lax" },
    ])
  }
  ...
  page.on("pageerror", (e) => consoleErrors.push({ name, text: `pageerror: ${e.message}` }))
  page.on("response", (r) => {
    if (r.status() >= 400) failedRequests.push({ name, status: r.status(), url: r.url() })
  })
...
try {
  ...
  heading("Pass 1 — first impression, cold, /de at 1440")
  {
    ...
    note(1, "above the fold captured", await s.shot("pass01-landing-1440-above-fold"))

    // Scroll slowly, the way a reader would.
    for (let i = 1; i <= 6; i += 1) {
      await s.page.evaluate((n) => window.scrollTo(0, (document.body.scrollHeight / 7) * n), i)
  ...
  heading("Pass 2 — the evidence claim: can I trace a number in 10 seconds?")
  {
    ...
    if ((await conflicted.count()) > 0) {
      ...
      note(2, "conflicted figure", JSON.stringify((await conflicted.innerText()).slice(0, 80)))
  ...
  for (const locale of ["de", "en", "tr", "ru"]) {
    for (const [w, h] of [[1440, 900], [375, 812]]) {
      const s = await session(`pass3-${locale}-${w}`, { locale, width: w, height: h })
      ...
      const rawKeys = [...body.matchAll(/\b(landing|hotel|evidence|dashboard|common)\.[a-z][a-zA-Z.]+/g)].map((m) => m[0])

      note(3, `${locale} @${w}`, `overflow ${overflow.scroll}>${overflow.client ? overflow.client : "?"} ${overflow.scroll > overflow.client ? "YES" : "no"} · tofu ${tofu} · raw keys ${[...new Set(rawKeys)].length}`)
      if (rawKeys.length > 0) note(3, `  raw keys in ${locale}`, [...new Set(rawKeys)].slice(0, 6).join(", "))
  ...
  heading("Pass 4 — every role's first screen")
  {
    ...
    for (const role of roles) {
      ...
      const response = await s.page.goto(`${BASE}/de/dashboard`, { waitUntil: "domcontentloaded" })
      await s.page.waitForTimeout(900)
      const navLinks = await s.page.locator("nav a[href*='/dashboard']").count()
      const text = await s.page.locator("body").innerText()
      note(4, role, `HTTP ${response?.status()} · ${navLinks} nav link(s) · ${text.trim().length} chars`)
  ...
  heading("Pass 5 — the 656-unit table")
  {
    ...
    const response = await s.page.goto(`${BASE}/de/dashboard/units`, { waitUntil: "domcontentloaded" })
    await s.page.waitForTimeout(1200)
    note(5, "units route", `HTTP ${response?.status()}`)
    await s.shot("pass05-units-top")
    await s.shotFull("pass05-units-full")

    const modelled = await s.page.locator("[data-data-quality='modelled'], [data-quality='modelled']").count()
    const portal = await s.page.locator("[data-data-quality='portal_listing'], [data-quality='portal_listing']").count()
  ...
  heading("Pass 6 — the conflict, end to end")
  {
    ...
    const response = await s.page.goto(`${BASE}/de/dashboard/evidence`, { waitUntil: "domcontentloaded" })
    await s.page.waitForTimeout(1200)
    ...
    note(6, "USD kept as USD", /239[.,]171\s*\$|239[.,]171[^€]{0,30}(\$|USD)/.test(body) ? "yes" : "NO")
    note(6, "a converted EUR twin appears", /239[.,]171\s*€/.test(body) ? "YES — BAD" : "no")
    await s.close()
  }
  ...
  heading("Pass 11 — reduced motion: is any content missing?")
  {
    ...
    await page.screenshot({ path: join(OUT, "pass11-reduced-motion-full.png"), fullPage: true })
    await context.close()
  }
  ...
  {
    const s = await session("pass12", { role: "manager", locale: "de", width: 375, height: 812, mobile: true })
    for (const [label, path] of [["landing", "/de"], ["dashboard", "/de/dashboard"], ["units", "/de/dashboard/units"]]) {
      ...
      note(12, `${label} @375`, `scrollWidth ${overflow.scroll} vs clientWidth ${overflow.client}`)
    }
    await s.close()
  }
} finally {
...
writeFileSync(
  ...
  JSON.stringify({ base: BASE, slowMo: SLOW_MO, headless: HEADLESS, notes, consoleErrors, failedRequests }, null, 2),
  "utf8",
)
...
for (const e of [...new Map(consoleErrors.map((e) => [e.text, e])).values()].slice(0, 10)) {
  console.log(`  [${e.name}] ${e.text.slice(0, 140)}`)
}
console.log(`failed requests: ${failedRequests.length}`)
for (const r of [...new Map(failedRequests.map((r) => [r.url, r])).values()].slice(0, 10)) {

...
const BANNED_VOCAB = [
  ...
  {
    re: /belegte angaben/i,
    why: '"belegt" reads as OCCUPIED. Say what is missing, not what is "unproven".',
  },
  {
    re: /\bmodelliert\b/i,
    why: 'Say what it means: "Preis nicht von der Quelle bestätigt".',
  },
  { re: /cockpit/i, why: "Internal jargon. Use „Quellen und Nachweise“." },
  {
    re: /\bdeals\b/i,
    why: "Untranslated English among German labels. Use „Abschlüsse“.",
  },
  { re: /\bQA\b/, why: "QA is our word, not the user's." },
  {
    re: /demo-daten/i,
    why: 'Jargon. Use "Beispieldaten" and keep the honesty.',
  },
  {
    re: /virtualisiert|virtualised|virtualized/i,
    why: "Implementation detail.",
  },
  { re: /synthetische|synthetic/i, why: "Developer word." },
  {
    re: /\bDataTable\b|\bdata table\b/i,
    why: "A component name, not a thing a user has heard of.",
  },
  {
    re: /\bZeilen\b|\brows\b/i,
    why: 'Nobody outside engineering says "rows". Use "Einheiten".',
  },
  {
    re: /^(ready|loading|empty|error)$/i,
    why: "State-machine names are not button labels.",
  },
];
...
function walkJson(value, keyPath, visit) {
  ...
  if (Array.isArray(value)) {
    value.forEach((v, i) => walkJson(v, `${keyPath}[${i}]`, visit));
    return;
  }
  if (value && typeof value === "object") {
    for (const [k, v] of Object.entries(value))
      walkJson(v, keyPath ? `${keyPath}.${k}` : k, visit);
  }
}
...
async function checkMessages() {
  ...
  for (const locale of LOCALES) {
    ...
    if (!existsSync(file)) {
      add("messages-missing", file, 0, `no ${locale}.json`, "");
      continue;
    }
    ...
    walkJson(doc, "", (keyPath, value) => {
      ...
      if (dash) {
        add(
          ...
          file,
          lineNo,
          `${keyPath} contains ${dash === EM_DASH ? "an em dash" : `a "${dash}" in prose`}`,
          value,
        );
      }
...
async function checkTsx() {
  const files = [
    ...(await tsxFiles(path.join(WEB, "app"))),
    ...(await tsxFiles(path.join(WEB, "components"))),
  ];
    ...
    lines.forEach((line, idx) => {
      ...
      if (lineDash) {
        add(
          ...
          file,
          lineNo,
          `${lineDash === EM_DASH ? "em dash" : `"${lineDash}" in prose`} in code`,
          line.trim(),
        );
      }
      ...
      for (const m of line.matchAll(
        ...
        add(
          ...
          file,
          lineNo,
          `${prop}="${value}" is user-visible and not from messages/*`,
          value,
        );
      }
      ...
      for (const m of line.matchAll(/>([^<>{}\n]{3,})</g)) {
        ...
        add(
          ...
          file,
          lineNo,
          `JSX text "${value}" is not from messages/*`,
          value,
        );
      }
    });
    ...
    linesOf(code).forEach((line, idx) => {
      ...
      if (d)
        add(
          ...
          file,
          idx + 1,
          `${d === EM_DASH ? "em dash" : `"${d}" in prose`} in shell copy`,
          line.trim(),
        );
...
console.log(
  `  scanned : ${stringCount} message strings across ${LOCALES.length} locales, ${fileCount} .tsx files`,
);
console.log("");
...
if (findings.length === 0) {
  console.log(
    "  0 findings. Every user-visible string is in messages/*, and none contains an em dash.",
  );
  process.exit(0);
}
console.log(
  `  ${findings.length} finding(s). Run with --list to see each one.`,
);
process.exit(1);

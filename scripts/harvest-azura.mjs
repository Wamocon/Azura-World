...
function transportLabelFor(errorMessage) {
  ...
  if (
    ...
    m.includes("err_name_resolution_failed")
  )
    return "dns_timeout";
  if (m.includes("err_connection_timed_out") || m.includes("err_timed_out"))
    return "dns_timeout";
  if (m.includes("err_cert") || m.includes("ssl") || m.includes("err_bad_ssl"))
    return "tls_invalid";
  if (
...
function redirectVerdict(requestedUrl, finalUrl, source) {
  ...
  if (from.startsWith(to))
    return `redirected_to_ancestor(${from} → ${to || "/"})`;
...
function validateBody({
  ...
  if (Buffer.byteLength(html, "utf8") < minBytes) {
    reasons.push(
      `body_under_min_bytes(${Buffer.byteLength(html, "utf8")}<${minBytes})`,
    );
  }
  ...
  const tokenOk =
    mode === "all" ? hits.length === tokens.length : hits.length > 0;
...
function statusLabel({ httpStatus, reasons, transport }) {
  ...
  if (reasons.some((r) => r.startsWith("redirected_to_ancestor")))
    return "soft_404_redirect";
  if (reasons.some((r) => r.startsWith("redirected_off_path")))
    return "redirected";
  if (reasons.some((r) => r.startsWith("body_under_min_bytes")))
    return "thin_body";
  if (reasons.some((r) => r.startsWith("expect_missing")))
...
async function captureSource({
  ...
  const candidates = [source.url, ...(source.altUrls || [])].filter(
    (u) => u && u !== "UNRESOLVED",
  );
  ...
  for (const candidate of candidates) {
    ...
    // ---- navigate ---------------------------------------------------------
    const tlsPasses =
      source.tlsSuspect && ARGS.allowInvalidTls ? [false, true] : [false];
    ...
    for (const ignoreHTTPSErrors of tlsPasses) {
      ...
      let context = null;
      try {
        context = await newRealisticContext(
          browser,
          source,
          defaults,
          ignoreHTTPSErrors,
        ...
        await page
          .waitForLoadState("networkidle", {
            timeout: defaults.settleTimeoutMs,
          })
          .catch(() => {});
        ...
        const saved = await persistCapture({
          source,
          url: candidate,
          page,
          html,
...
async function loadRobots(requestFactory, origin) {
  try {
    ...
    return {
      status: `unavailable:${transportLabelFor(error?.message)}`,
      groups: null,
    };
  }
}

async function newRealisticContext(
  browser,
  source,
  defaults,
  ignoreHTTPSErrors,
  ...
  const localeMap = {
    de: {
      locale: "de-DE",
      accept: "de-DE,de;q=0.9,en;q=0.6",
      tz: "Europe/Berlin",
    },
    en: { locale: "en-US", accept: "en-US,en;q=0.9", tz: "Europe/Istanbul" },
    tr: {
      locale: "tr-TR",
      accept: "tr-TR,tr;q=0.9,en;q=0.6",
      tz: "Europe/Istanbul",
    },
    ru: {
      locale: "ru-RU",
      accept: "ru-RU,ru;q=0.9,en;q=0.6",
      tz: "Europe/Moscow",
    },
  };
...
async function dismissConsent(page, defaults, source) {
  const selectors = [
    ...(defaults.consentSelectors || []),
    ...(source.consentSelectors || []),
  ];
...
async function persistCapture({ source, url, page, html, text }) {
  ...
  await page
    .screenshot({ path: pngPath, fullPage: true, timeout: 25000 })
    .catch(async () => {
      await page
        .screenshot({ path: pngPath, fullPage: false, timeout: 15000 })
        .catch(() => {});
    });
  await writeFile(
    metaPath,
...
function diffManifests(prev, next) {
  ...
  for (const entry of next.entries) {
    ...
    if (
      before.snapshotHash &&
      entry.snapshotHash &&
      before.snapshotHash !== entry.snapshotHash
    ) {
      changes.push({
        id: entry.id,
        kind: "content_changed",
        from: before.snapshotHash.slice(0, 12),
        to: entry.snapshotHash.slice(0, 12),
      });
    }
...
async function main() {
  ...
  const defaults = {
    ...
    maxGlobalConcurrency:
      ARGS.concurrency || config.defaults.maxGlobalConcurrency,
  };

  let selected = [...config.sources];
  if (ARGS.units) selected = selected.concat(config.unitListingSources || []);
  if (ARGS.only.length)
    selected = selected.filter((s) => ARGS.only.includes(s.id));
  ...
  console.log(
    `harvest-azura · ${selected.length} sources across ${hostGroups.size} hosts`,
  );
  console.log(
  ...
  console.log(
    `  tls tolerance: ${ARGS.allowInvalidTls ? "ENABLED (--allow-invalid-tls)" : "disabled"}`,
  );
  ...
  if (ARGS.dryRun) {
    ...
    for (const [host, sources] of hostGroups) {
      ...
      for (const s of sources) {
        console.log(
          `    ${s.id.padEnd(28)} tier ${s.tier}  ${s.locale}  ${s.urlUnresolved ? "UNRESOLVED" : s.url}`,
        );
      }
    }
    process.exit(0);
  ...
  if (headedBrowser)
    console.log(
      "  launched a second, headed browser for requiresHeaded sources",
    );
  const browserFor = (source) =>
    source.requiresHeaded && headedBrowser ? headedBrowser : browser;
  ...
  const worker = async () => {
    while (cursor < queue.length) {
      ...
      for (const source of sources) {
        ...
        entry.headed =
          ARGS.headed || Boolean(source.requiresHeaded && headedBrowser);
        entries.push(entry);
        ...
        if (wall) {
          ...
          if (consecutiveWalls >= 2) {
            softBanned.add(host);
            console.log(
              `  ! ${host}: two consecutive walls — backing off, remaining sources not attempted`,
            );
          } else {
  ...
  const workers = Array.from(
    { length: Math.min(defaults.maxGlobalConcurrency, queue.length) },
    worker,
  );
  ...
  for (const parent of followParents) {
    ...
    if (!parentEntry?.snapshotPath || !parentEntry.contentValidated) {
      console.log(
        `  follow ${parent.id}: skipped (container page not validated)`,
      );
      continue;
    }

    const html = await readFile(
      path.join(ROOT, parentEntry.snapshotPath),
      "utf8",
    );
    ...
    console.log(
      `  follow ${parent.id}: ${found.size} link(s) matched, harvesting ${targets.length}`,
    );
    ...
    for (const [index, target] of targets.entries()) {
      ...
      childEntry.headed =
        ARGS.headed || Boolean(child.requiresHeaded && headedBrowser);
  ...
  const configIndex = new Map(
    [...(config.sources || []), ...(config.unitListingSources || [])].map(
      (s) => [s.id, s],
    ),
  );

  const tlsSuspects = allEntries.filter(
    (e) => configIndex.get(e.id)?.tlsSuspect,
  );
  ...
  if (unresolved.length) {
    findings.push({
      ...
      resolution:
        'Left as a gap. Facts that depended on these sources stay confidence: "gap".',
      resolvedTo: null,
    });
  }
  ...
  if (disallowed.length) {
    findings.push({
      ...
      resolution:
        "Respected. The source is recorded as unavailable, not as empty.",
      resolvedTo: null,
    });
  }

  const dead = allEntries.filter((e) => !e.contentValidated);
  const tierAuthority = allEntries.filter(
    (e) => e.tier <= 3 && e.contentValidated,
  );
  findings.push({
  ...
  const manifest = {
    ...
    counts: {
      ...
      notAttempted: allEntries.filter((e) => e.status === "not_attempted")
        .length,
    },
  ...
  await writeFile(
    path.join(MANIFEST_ARCHIVE, `${runId}.json`),
    JSON.stringify(manifest, null, 2),
    "utf8",
  );
  await writeFile(
    FINDINGS_PATH,
    JSON.stringify({ generatedAt: nowIso(), findings }, null, 2),
    "utf8",
  );
  await writeFile(
    DIFF_PATH,
    JSON.stringify(
      { generatedAt: nowIso(), previousRun: prev?.runId ?? null, ...diff },
      null,
      2,
    ),
    "utf8",
  ...
  console.log(
    `  unit pages    : ${manifest.counts.discoveredUnitPages} discovered by link-following`,
  );
  console.log(
    `  validated     : ${manifest.counts.validated}/${manifest.counts.total}`,
  );
  ...
  if (diff.baseline) {
    ...
    for (const c of diff.changes.slice(0, 20)) {
      console.log(
        `    ${c.id.padEnd(28)} ${c.kind} ${c.from ?? ""} → ${c.to ?? ""}`,
      );
    }
  }

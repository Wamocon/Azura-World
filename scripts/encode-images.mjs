...
import { DIR, REPO, loadDep, resolveUsage, loadRightsPolicy } from './harvest-media.mjs'
...
export async function encodeAsset(asset, { outDir, urlPrefix, dryRun = false }) {
  ...
  if (!RASTER_KINDS.has(asset.kind))
    return { skipped: `not_raster(${asset.kind})`, formats: { avif: [], webp: [], jpeg: [] }, lqip: null, bytes: 0 }

  let meta
  try {
    meta = await sharp(src).metadata()
  } catch (e) {
    return { skipped: `metadata_failed: ${String(e.message).split('\n')[0]}`, formats: { avif: [], webp: [], jpeg: [] }, lqip: null, bytes: 0 }
  }
  ...
  const srcH = isVector ? Math.round(WIDTHS[WIDTHS.length - 1] / ((meta.width ?? 1) / (meta.height ?? 1) || 1)) : meta.height
  if (!srcW || !srcH) return { skipped: 'no_dimensions', formats: { avif: [], webp: [], jpeg: [] }, lqip: null, bytes: 0 }
  ...
  const widths = [...new Set(WIDTHS.filter((w) => w <= srcW).concat(Math.min(srcW, WIDTHS[0])))].sort((a, b) => a - b)
        ...
        const out =
          fmt === 'avif'
            ...
            : fmt === 'webp'
              ...
              : pipe.flatten({ background: '#ffffff' }).jpeg({ quality: QUALITY.jpeg, mozjpeg: true })
...
async function run() {
  ...
  if (!existsSync(assetsFile)) {
    console.error(`sources/media/assets.json not found. Run: node scripts/harvest-media.mjs`)
    process.exitCode = 1
    return
  }
  ...
  console.log(`  assets        : ${assets.length} from sources/media/assets.json`)
  console.log(`  formats       : AVIF q${QUALITY.avif} · WebP q${QUALITY.webp} · JPEG q${QUALITY.jpeg}`)
  console.log(`  widths        : ${WIDTHS.join(' / ')} (never upscaled)`)
  console.log(`  LQIP          : ${LQIP_WIDTH}px blurred WebP data URI`)
  console.log(`  EXIF          : stripped from every output (no withMetadata anywhere)`)
  ...
  if (!dryRun) {
    ...
    await writeFile(
      ...
      JSON.stringify(
        {
          __generated: 'scripts/encode-images.mjs — DO NOT HAND-EDIT. Fix the encoder and re-run.',
    ...
    await writeFile(path.join(DIR.media, 'encoded.json'), JSON.stringify({ generatedAt: new Date().toISOString(), generator: 'scripts/encode-images.mjs', encoded, skipped }, null, 2))
  }
  ...
  console.log(`\n── Encoded ──`)
  console.log(`  assets encoded      : ${encoded.length}  (public ${publicCount} · internal ${internalCount})`)
  ...
  for (const [label, d] of [
    ...
    for (const [e, b] of Object.entries(d.byExt).sort((a, b) => b[1] - a[1])) console.log(`      ${e.padEnd(6)} ${fmt(b).padStart(10)}`)
    console.log(`      TOTAL  ${fmt(d.total).padStart(10)} across ${d.files} files`)
  }

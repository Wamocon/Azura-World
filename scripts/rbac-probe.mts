...
equal(
  "allPermissions length (21 resources × 8 actions)",
...
section("[DoD 6c] The module-load guard fires in a real process");
{
  ...
  const registerUrl = new URL("./register-ts-resolve.mjs", import.meta.url)
    .href;
  ...
  check(
    ...
    stderr
      .split("\n")
      .find((line) => line.includes("AccessProfile"))
      ?.trim() ?? "(no matching stderr line)",
  );
  check(
  ...
  check(
    ...
    `exit status ${String(control.status)}${
      control.status === 0
        ? ""
        : ` — ${(control.stderr ?? "").trim().slice(0, 200)}`
    }`,
  );
}
...

{
  ...
  equal(
    "profile from unknown cookie: source",
    unknown.source,
    "access-profile",
  );
  check(
...

{
  ...
  equal(
    "control: a valid admin row resolves to admin",
    realAdmin.role,
    "admin",
  );
  equal(
    "control: a valid row sets no degradedReason",
    realAdmin.degradedReason,
    null,
  );
  check(
    "control: admin may evidence:manage",
    profileCan(realAdmin, "evidence:manage"),
  );
...
for (const child of ["child_owner", "child_tenant", "child_guest"] as const) {
  for (const forbidden of [
    ...
    check(`${child} may NOT ${forbidden}`, !hasPermission(child, forbidden));
  }
  check(

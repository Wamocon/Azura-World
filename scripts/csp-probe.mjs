...
if (!Number.isInteger(port) || port < 1 || port > 65535) {
  console.error(`--port must be an integer 1-65535, got ${argv[portArgIndex + 1]}`)
  process.exit(2)
}
...
function phaseBuildArtefacts() {
  ...
  const htmlRoutes = prerendered.filter((route) =>
    existsSync(join(nextDir, "server", "app", `${route.replace(/^\//, "")}.html`))
  )
  const nonHtmlRoutes = prerendered.filter((route) => !htmlRoutes.includes(route))

  console.log(
    `  prerendered routes: ${prerendered.length === 0 ? "(none)" : prerendered.join(", ")}`
  )
  if (nonHtmlRoutes.length > 0) {
    console.log(`  …of which emit no HTML document: ${nonHtmlRoutes.join(", ")}`)
  }
  ...
  if (htmlRoutes.length === 0) {
    check("no prerendered HTML route at all", true, "the strongest possible state")
  }
    ...
    const attempt = () => {
      ...
      socket.once("error", () => {
        ...
        if (Date.now() > deadline) {
          reject(new Error(`nothing listening on 127.0.0.1:${targetPort} after ${timeoutMs}ms`))
          return
        }
...
function startServer() {
  ...
  const child = spawn(
    ...
    [nextBin, "start", "--hostname", "127.0.0.1", "--port", String(port)],
    {
      ...
      env: { ...process.env, NODE_ENV: "production", AZURA_ENABLE_KITCHEN_SINK: "1" },
    }
  )
  const log = []
...
async function phaseRuntime(base) {
  section("Phase 2 — runtime under `next start`: every script must carry the nonce")
  ...
  for (const probe of PROBE_PAGES) {
    ...
    check(`${probe.path} sends a Content-Security-Policy`, csp !== null, `status ${response.status}`)
    if (csp === null) continue

    const nonce = nonceFromCsp(csp)
    check(`${probe.path} CSP carries a nonce`, nonce !== null, csp.slice(0, 90))
    check(
    ...
    check(`${probe.path} HTML ships script tags`, tags.length > 0, `${tags.length} <script> tags`)
    ...
    for (const probe of PROBE_PAGES) {
      ...
      const inPage = await page.evaluate(() => {
        ...
        return {
          ...
          jsDecoded: scripts.reduce((sum, entry) => sum + entry.decodedBodySize, 0),
          violations: globalThis.__cspViolations ?? [],
          scriptTags: document.querySelectorAll("script").length,
          noncedTags: document.querySelectorAll("script[nonce]").length,
          hydrated: Object.keys(document.body).some((key) => key.startsWith("__react")),
        }
      })
      ...
      check(
        ...
        allViolations.length === 0 ? "0 violations" : `${allViolations.length}: ${allViolations[0]}`
      )
      check(
...
console.log(
  failures === 0 ? `${bold(c("32", "OK"))}  ${summary}` : `${bold(c("31", "FAILED"))}  ${summary}`
)

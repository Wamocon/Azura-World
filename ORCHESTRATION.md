...
| **W2-A** | `apps/web/lib/*-repository.ts`, `apps/web/lib/*-data.ts` *(except `azura-world-data.ts`)*, `apps/web/lib/seed-data.ts` |
...
Each window is one *task*, but a task is not one *thread*. Fan out inside your window with
...
| Task | Fan out on |
|---|---|
| W0-A | contracts transcription · env+proxy+layout · docs+setup script *(after `pnpm install`, which is serial and first)* |
